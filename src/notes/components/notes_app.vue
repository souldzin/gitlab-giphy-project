...
export default {
  ...
  computed: {
    ...
    allDiscussions() {
      ...
      if (this.renderSkeleton || this.isLoading) {
        const prerenderedNotesCount =
          parseInt(this.notesData.prerenderedNotesCount, 10) || 0;
    ...
    canReply() {
      return (
        this.userCanReply && !this.commentsDisabled && !this.timelineEnabled
      );
    },
    slotKeys() {
  ...
  mounted() {
    ...
    eventHub.$on(
      "notesApp.updateIssuableConfidentiality",
  ...
  methods: {
    ...
    getFetchDiscussionsConfig() {
      ...
      const currentFilter =
        this.getNotesDataByProp("notesFilter") ||
        constants.DISCUSSION_FILTERS_DEFAULT_VALUE;
      ...
      <template #comments>
        <ul id="notes-list" class="notes main-notes-list timeline">
          <template v-for="discussion in allDiscussions">
            ...
            <template
              v-else-if="discussionIsIndividualNoteAndNotConverted(discussion)"
            >
              <system-note
